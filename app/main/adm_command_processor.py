...
from main.http.webpage_proxy import ADMProxy
...
class ADMCommandProcessor:
    ...
    def action_cli_request(self, cli_dict):
        ...
        if function_to_call == LOCATIONS:
            if project:
                result = self.adm_proxy.get_locations_for_project(project)
            elif group:
                result = self.adm_proxy.get_locations_for_group(group)
            else:
                result = self.adm_proxy.get_locations()
            data_type = DataType.adm_locations
        elif function_to_call == CONFIGS:
            if project:
                result = self.adm_proxy.get_configs_for_project(project)
            elif group:
                result = self.adm_proxy.get_configs_for_group(group)
            else:
                result = self.adm_proxy.get_configs()
            data_type = DataType.adm_configs
        elif function_to_call == VERSIONS:
            if project:
                result = self.adm_proxy.get_versions_for_project(project)
            elif group:
                result = self.adm_proxy.get_versions_for_group(group)
            else:
                result = self.adm_proxy.get_versions()
            data_type = DataType.adm_versions
        elif function_to_call == SCRIPTS:
            if project:
                result = self.adm_proxy.get_scripts_for_project(project)
            elif group:
                result = self.adm_proxy.get_scripts_for_group(group)
            else:
                result = self.adm_proxy.get_scripts()
            data_type = DataType.adm_scripts
        elif function_to_call == ARTIFACTS:
            if project:
                result = self.adm_proxy.get_artifacts_for_project(project)
            elif group:
                result = self.adm_proxy.get_artifacts_for_group(group)
            else:
                result = self.adm_proxy.get_artifacts()
